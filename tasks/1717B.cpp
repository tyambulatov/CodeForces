...
int main()
{
    ...
    while (t--)
    {
        ...
        vector<vector<char>> baseSquare(k, vector<char>(k, '.'));
        set<int> Xtaken = {(r - 1) % k};
        set<int> Ytaken = {(c - 1) % k};
        baseSquare[(r - 1) % k][(c - 1) % k] = 'X';
        ...
        forn(i, k)
        {
            forn(j, k)
            {
                if (baseSquare[i][j] == '.')
                {
                    if (Xtaken.count(i) == 0 && Ytaken.count(j) == 0)
                    {
                        baseSquare[i][j] = 'X';
                        Xtaken.insert(i);
                        Ytaken.insert(j);
